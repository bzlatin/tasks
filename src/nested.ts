...
export function addNewQuestion(
    ...
    // Return a new array with the new question added to the end
...
export function duplicateQuestionInArray(
    ...
    questions.forEach((question) => {
        result.push(question); // Add the current question to the result array
        if (question.id === targetId) {
            // If the current question is the one to duplicate
            const duplicate = duplicateQuestion(newId, question);
            result.push(duplicate); // Add the duplicate immediately after
        }
    });
    return result;
}
